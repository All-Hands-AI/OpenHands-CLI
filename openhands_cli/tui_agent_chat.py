...
class TUIAgentChat:
    ...
    def _handle_command(self, command: str) -> bool:
        ...
        if command_lower == "/exit":
            ...
            self.tui.add_html_output(
                "<yellow>Settings screen not yet integrated with pinned input TUI</yellow>"
            )
            return True
            ...
            self.tui.add_html_output(
                "<yellow>MCP screen not yet integrated with pinned input TUI</yellow>"
            )
            return True
            ...
            try:
                ...
                self.tui.add_html_output(
                    f"<red>Error starting fresh conversation: {e}</red>"
                )
            return True
            ...
            if self.conversation is not None:
                ...
                self.tui.add_html_output(
                    "<yellow>Status display not yet integrated with pinned input TUI</yellow>"
                )
            else:
            ...
            if not self.runner:
                self.tui.add_html_output(
                    "<yellow>No active conversation running...</yellow>"
                )
                return True
            ...
            if not (
                ...
                self.tui.add_html_output(
                    "<red>No paused conversation to resume...</red>"
                )
                return True
    ...
    def _process_message(self, message: Message | None) -> None:
        ...
        if not self.runner or not self.conversation:
            ...
            self.runner = ConversationRunner(
                self.conversation, on_agent_finished=self._on_agent_finished
            )
