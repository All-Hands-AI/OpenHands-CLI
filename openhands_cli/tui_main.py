...
class OpenHandsTUI:
    ...
    def _setup_ui(self) -> None:
        ...
        output_focused = Condition(
            lambda: self.app and self.app.layout.current_buffer == self.output_buffer
        )
        ...
        for key in editing_keys:
            ...
            def prevent_output_edit(_event, key=key):
    ...
    async def _handle_user_input(self) -> None:
        ...
        if command == "/exit":
            ...
            self._add_output(
                "⚙️  Settings screen would open here (not implemented in TUI yet)"
            )
            return

        elif command == "/mcp":
            self._add_output(
                "🔌 MCP information would be shown here (not implemented in TUI yet)"
            )
            return
    ...
    async def _process_message(self, message: Message | None) -> None:
        ...
        try:
            ...
            if not self.runner or not self.conversation:
                self.conversation = self._setup_conversation_with_tui_visualizer(
    ...
    def _setup_conversation_with_tui_visualizer(self, conversation_id):
        ...
        from openhands.sdk import BaseConversation, Conversation, Workspace
    ...
    def show_confirmation_dialog(
        self, question: str, options: list[str], pending_actions: list = None
    ) -> int | None:
        """Show confirmation dialog using prompt_toolkit's native radiolist_dialog."""
        # Build the dialog text with pending actions info
        dialog_text = question
        if pending_actions:
            dialog_text += f"\n\nAgent created {len(pending_actions)} action(s):"
            for i, action in enumerate(pending_actions[:3], 1):  # Show first 3 actions
                tool_name = getattr(action, "tool_name", "[unknown tool]")
                action_content = (
                    str(getattr(action, "action", ""))[:60].replace("\n", " ")
                    or "[unknown action]"
                )
                dialog_text += f"\n  {i}. {tool_name}: {action_content}..."
            if len(pending_actions) > 3:
                dialog_text += f"\n  ... and {len(pending_actions) - 3} more"

        # Create radio list values (value, label) pairs
        values = [(i, option) for i, option in enumerate(options)]

        # Show the native radiolist dialog
        try:
