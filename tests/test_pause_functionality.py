...
from openhands_cli.agent_chat import ConversationRunner, UserConfirmation, ask_user_confirmation
...
class TestPauseFunctionality:
    ...
    def test_pause_in_confirmation_mode_returns_to_main_input(self):
        ...
        self.mock_conversation.state.confirmation_mode = True
        self.mock_conversation.state.agent_waiting_for_confirmation = False
    ...
    def test_keyboard_interrupt_during_confirmation_pauses_agent(self, mock_ask_confirmation):
        ...
        with patch('openhands_cli.agent_chat.get_unmatched_actions') as mock_get_actions:
    ...
    def test_resume_or_any_message_from_main_input_resumes_agent(self):
        ...
        from openhands.sdk import Message, TextContent
        resume_message = Message(role="user", content=[TextContent(text="/resume")])
        ...
        any_message = Message(role="user", content=[TextContent(text="please continue")])
    ...
    @patch('openhands_cli.agent_chat.ask_user_confirmation')
    def test_resume_with_confirmation_mode_shows_confirmation_options(self, mock_ask_confirmation):
        ...
        with patch('openhands_cli.agent_chat.get_unmatched_actions') as mock_get_actions:
            ...
            with patch('openhands_cli.agent_chat.pause_listener') as mock_pause_listener:
                ...
                from openhands.sdk import Message, TextContent
                message = Message(role="user", content=[TextContent(text="please continue")])
...
class TestPauseListener:
    ...
    def test_pause_listener_initialization(self):
        ...
        assert listener.confirmation_mode is False
        assert not listener.is_paused()

    def test_pause_listener_with_confirmation_mode(self):
        """Test PauseListener initialization with confirmation mode."""
        mock_callback = MagicMock()
        listener = PauseListener(on_pause=mock_callback, confirmation_mode=True)
        
        assert listener.confirmation_mode is True
